...
//_gentauTag     (consumes<edm::View<pat::GenericParticle>>        (iConfig.getParameter<edm::InputTag>("genPartCollection"))),
...
void Ntuplizer::analyze(const edm::Event& iEvent, const edm::EventSetup& eSetup)
{
    ...
    if(_isMC)
      ...
      //iEvent.getByToken(_gentauTag, genPartHandle);}
	...
	if(abs((*genPartHandle)[gt].pdgId())==15){
	  ...
	  _gentauPt  = (*genPartHandle)[gt].pt();
	  _gentauEta = (*genPartHandle)[gt].eta();
	  _gentauPhi = (*genPartHandle)[gt].phi();
	}    
      }
    }
